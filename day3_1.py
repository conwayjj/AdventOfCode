...
def traceRoute(wire, command, start, length):
    direction = command[0]
    ...
    for i in range(distance):
        length += 1
        curLoc = (curLoc[0] + x, curLoc[1] + y)
        if curLoc in grid:
            grid[curLoc] += wire + str(length)
        else:
            grid[curLoc] = wire + str(length)
    return curLoc, length

def parseLength(inString, wire):
    startIndex = inString.find(wire) + 1
    stopIndex = startIndex
    while stopIndex < len(inString) and inString[stopIndex].isdigit():
        stopIndex += 1
##    if stopIndex == len(inString) - 1:
##        print(startIndex, stopIndex)
##        outStr = inString[startIndex:]
##    else:
    print(startIndex,stopIndex)
    outStr = inString[startIndex:stopIndex]
    print(inString, startIndex, stopIndex, outStr)
    return(int(outStr))

curLoc = (0,0)
length = 0
for com in aList:
    curLoc, length = traceRoute('a', com, curLoc, length)
curLoc = (0,0)
length = 0
for com in bList:
    curLoc, length = traceRoute('b', com, curLoc, length)

shortestPath = 9999999
shortestLen = 9999999
for loc in grid:
    if 'a' in grid[loc] and 'b' in grid[loc]:
        dist = abs(loc[0]) + abs(loc[1])
        aLen = parseLength(grid[loc], 'a')
        bLen = parseLength(grid[loc], 'b')
        totLen = aLen + bLen
        print("Crossing at:", loc, dist, grid[loc], aLen, bLen)
        if dist < shortestPath and dist > 0:
            shortestPath = dist
        if totLen < shortestLen and totLen > 0:
            shortestLen = totLen

print("Shortest path:", shortestPath)
print("Shortest length:", shortestLen)
